...
- Git追跡済みの対象UTF-8ファイルだけ。最大2 MB・1,500ファイル・1ファイル100 KB。未追跡ファイルはローカルで別途確認し、選別のためだけにstageしません。
- 22 KBのバッチ許容量を超えるファイルは推論せず保持します。すべて未判定ならJev呼び出しは0回です。
- 依存解決は部分的です。workspace package、`.mts`、複雑なalias、Pythonの`src`配置には改善余地があります。
