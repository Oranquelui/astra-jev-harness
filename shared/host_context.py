"""Read-only Jev context handoff shared by native conversation hosts.

Each host adapter module supplies SURFACE, LABEL, NO_JEV_ROUTE, fresh(),
load_host_plan() and execution_environment(). They are looked up on the adapter
at call time so its public functions and test patches keep their meaning. The
host conversation implements; this core never generates code or edits sources.
"""
import argparse
import json
import os
...
def load_surface_plan(path, host):
    plan = rc.load_plan(path)
    if plan.get('surface') != host.SURFACE:
        raise ProtocolError(f'Expected a {host.LABEL} context plan')
    return plan


def load_surface_record(selection_dir, host, statuses):
    record = json.loads((Path(selection_dir) / 'selection.json').read_text())
    if record.get('surface') != host.SURFACE or record.get('status') not in statuses:
        raise ProtocolError(f'Expected a {host.LABEL} selection record; inspect saved attempts before retrying')
    return record


def child_call_fields(host):
    # Desktop keeps its published astra_child_calls field; other hosts use a neutral name.
    return {'astra_child_calls': 0} if host.SURFACE == 'desktop' else {'child_generation_calls': 0}


def make_plan(host, repo, task, out, include_paths=None, focus_paths=None, scope_max_calls=4):
    plan = rc.snapshot(repo, task, out, include_paths=include_paths,
                       focus_paths=focus_paths, scope_max_calls=scope_max_calls)
    out = Path(out).resolve()
    out.chmod(0o700)
    plan['surface'] = host.SURFACE
    write_json(out / 'plan.json', plan)
    (out / 'PLAN.md').write_text('\n'.join([
        f'# {host.LABEL} context plan', '', f"Repository: {plan['repo']}", f"HEAD: {plan['head']}",
...
def select(host, plan_dir, out, max_calls=4, policy='batch', cache_dir=None, evidence_dir=None, mode='jev'):
    ...
    env, source = host.execution_environment() if required else ({}, 'not_needed')
    ...
    record['seconds'] = time.monotonic()-started
    write_json(result_path, record)
    child_line = ('Astra child calls: 0.' if host.SURFACE == 'desktop'
                  else 'Child generation calls: 0; this helper never generates code.')
    (out / 'REPORT.md').write_text(
        ...
        f'Report Jev usage separately from {host.LABEL} conversation usage.\n'
        + f"\nRoute: {recorded_route}. Policy: {policy}. Judged: {selected['metrics']['judged_files']}; "
...
def compare(host, selection_dir, required_paths=None):
    """Compare policies against identical saved evidence, without network or writes."""
    selection_dir = Path(selection_dir).resolve()
    record = load_surface_record(selection_dir, host, ('selected', 'failed', 'selecting'))
    plan_dir = Path(record['plan_dir'])
    if rc.digest((plan_dir / 'plan.json').read_bytes()) != record['plan_sha256']:
...
def main(host, description=None, argv=None):
    ...
    try:
        if args.command == 'doctor':
            env, source = host.execution_environment()
            print(json.dumps({'surface': host.SURFACE, 'harness_root': str(ROOT),
                              'key_source': source, 'typesafe_key_present': bool(env.get('TYPESAFE_API_KEY'))}))
            return 0 if env.get('TYPESAFE_API_KEY') else 1
        if args.command == 'plan':
            plan = host.make_plan(args.repo, Path(args.task_file).read_text(), args.out,
        ...
        print(f'{host.LABEL} context operation failed; inspect plan freshness, call budget, '
              'credential availability and saved selection status.', file=sys.stderr)
        return 1
